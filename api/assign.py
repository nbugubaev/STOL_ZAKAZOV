...
TG_MASTER_BOT_TOKEN = os.environ.get("TG_MASTER_BOT_TOKEN")
...
def assign_ticket(ticket_id, master_id, master_name, category):
...
def build_task_text(ticket):
    ...
    parts = [f"🛠 Новая задача №{int(ticket['ticket_no']):04d}" if ticket.get("ticket_no") else "🛠 Новая задача"]
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            ticket = assign_ticket(ticket_id, master_id, master.get("name"), category)

            inline = {"inline_keyboard": [[
                {"text": "✅ Выполнено", "callback_data": f"done:{ticket_id}"},
                {"text": "↩️ Отклонить", "callback_data": f"reject:{ticket_id}"},
            ]]}
            try:
